// Simulator Logic & Interactive Handlers
document.addEventListener('DOMContentLoaded', () => {
  ...
  function clearHighlights() {
    [modUser, modCatalog, modCart, modOrder, modPayment].forEach(mod => {
      if (mod) mod.classList.remove('active-request', 'faulty');
    });
  }
  ...
  window.simulateRequest = function(reqType) {
    if (!isSystemAlive) {
      addLog("Falla 503: La aplicacion Monolitica esta fuera de servicio. No responde a " + reqType + ".", 'error');
      return;
    }
    ...
    if (reqType === 'LOGIN') {
      ...
      if (modUser) modUser.classList.add('active-request');
      setTimeout(() => {
      ...
      if (modCatalog) modCatalog.classList.add('active-request');
      setTimeout(() => {
      ...
      if (modCart) modCart.classList.add('active-request');
      if (modOrder) modOrder.classList.add('active-request');
      if (modPayment) modPayment.classList.add('active-request');
  ...
  window.simulateRedeploy = function() {
    ...
    setTimeout(() => {
      isRedeploying = false;
      isSystemAlive = true;
      if (monolithBox) monolithBox.classList.remove('crashing');
      if (appStatus) {
  ...
  window.simulateCrash = function() {
    ...
    if (modPayment) modPayment.classList.add('faulty');
    if (monolithBox) monolithBox.classList.add('crashing');
